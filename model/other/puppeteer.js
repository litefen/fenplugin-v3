...
class Puppeteer {
    constructor() {
        ...
        if (!fs.existsSync('./plugins/xiaoxue-plugin/data')) {
            fs.mkdirSync('./plugins/xiaoxue-plugin/data')
        }

        this.createDir('./plugins/xiaoxue-plugin/data/html')
    }
    ...
    */
    async browserInit() {
        ...
        if (!this.browser) {
            logger.error('puppteer Chromium 启动失败')
            return false
        }
    ...
    */
    ...
    dealTpl(name, data) {
        ...
        let savePath = `./plugins/xiaoxue-plugin/data/html/${name}/${saveId}.html`

        /** 读取html模板 */
        if (!this.html[tplFile]) {
            this.createDir(`./plugins/xiaoxue-plugin/data/html/${name}`)
        ...
        data.resPath = `${_path}/plugins/xiaoxue-plugin/resources/`
