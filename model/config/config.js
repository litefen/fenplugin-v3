...
class config {
    constructor() {
        /** 默认配置 */
        this.defCfgPath = './plugins/fenplugin-v3/config/default_config/'
        this.default_config = {}

        /** 用户配置 */
        this.configPath = './plugins/fenplugin-v3/config/config/'
        this.config = {}
    ...
    */
    ...
    watch(file, app, name, type = 'default_config') {
        ...
        watcher.on('change', path => {
            ...
            logger.mark(`[修改粉酱v3插件配置文件][${type}][${app}][${name}]`)
     ...
     * 获取配置yaml
     * @param app 模块
     * @param type 默认配置-default_config 小雪插件配置-config
    ...
    async getcurrentplugininfo() {
        let plugininfo = {
            pluginname: await redis.get('Yz:fen:config:pluginname'),
            version: await redis.get('Yz:fen:config:version'),
            author: await redis.get('Yz:fen:config:author'),
            qq: await redis.get('Yz:fen:config:qq'),
            groupid: await redis.get('Yz:fen:config:groupid'),
            botname: await redis.get('Yz:fen:config:botname')
        }
        return plugininfo
    }
}
